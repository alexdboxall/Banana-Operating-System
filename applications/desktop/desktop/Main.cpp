...
uint32_t antWhiteLine[5000];
uint32_t antBlackLine[5000];
...
void init()
{
	...
	for (int i = 0; i < 5000; ++i) {
		antBlackLine[i] = encodeDesktopColour(i & 1 ? 0xFFFFFF : 0x000000, false);
		antWhiteLine[i] = encodeDesktopColour(i & 1 ? 0x000000 : 0xFFFFFF, false);
	}
...
void desktopUpdateSingleScanlineInRange(int scanline, int minX, int maxX)
{
	int args[4];
	args[0] = scanline * desktopWidth + minX;
	args[1] = maxX - minX;
	args[2] = minX;
	args[3] = maxX;

	SystemCall((size_t) SystemCallNumber::WSBE, LINKCMD_RESUPPLY_DESKTOP, (size_t) args, (size_t) (desktopBuffer + args[0]));
}

int prevAntScanlines[4];
int prevAntMinX;
int prevAntMaxX;

uint32_t oldAntAreas[4][4096];
uint32_t moreOldAntStuff[2][4096][2];

void renderAnts(bool xChanged, bool yChanged, int antCurrentX, int antCurrentY, bool clearAnts)
{
	int antMinX = antBaseX < antCurrentX ? antBaseX : antCurrentX;
	int antMinY = antBaseY < antCurrentY ? antBaseY : antCurrentY;
	int antMaxX = antBaseX > antCurrentX ? antBaseX : antCurrentX;
	int antMaxY = antBaseY > antCurrentY ? antBaseY : antCurrentY;

	if (!firstAntDraw) {
		if (1 || antCurrentX <= antPrevX || antCurrentX <= antBaseX || antCurrentY <= antBaseY) {
			desktopUpdateSingleScanlineInRange(prevAntScanlines[0], prevAntMinX, prevAntMaxX);
			desktopUpdateSingleScanlineInRange(prevAntScanlines[1], prevAntMinX, prevAntMaxX);
		}
		desktopUpdateSingleScanlineInRange(prevAntScanlines[2], prevAntMinX, prevAntMaxX);
		desktopUpdateSingleScanlineInRange(prevAntScanlines[3], prevAntMinX, prevAntMaxX);

		if (1 || antCurrentY <= antPrevY || antCurrentX <= antBaseX) {
			for (int y = prevAntScanlines[0]; y < prevAntScanlines[2]; ++y) {
				desktopUpdateSingleScanlineInRange(y, prevAntMinX, prevAntMinX + 2);
			}
		}
		for (int y = prevAntScanlines[0]; y < prevAntScanlines[2]; ++y) {
			desktopUpdateSingleScanlineInRange(y, prevAntMaxX - 2, prevAntMaxX);
		}
	}
	...
	int args[4];
	args[0] = antMinY * desktopWidth + antMinX;
	args[1] = antMaxX - antMinX;
	args[2] = antMinX;
	args[3] = antMaxX;

	memcpy(oldAntAreas[0], desktopBuffer + args[0], args[1] * 4);
	SystemCall((size_t) SystemCallNumber::WSBE, LINKCMD_RESUPPLY_DESKTOP, (size_t) args, (size_t) (antBlackLine));
	memcpy(desktopBuffer + args[0], oldAntAreas[0], args[1] * 4);

	args[0] += desktopWidth;
	memcpy(oldAntAreas[1], desktopBuffer + args[0], args[1] * 4);
	SystemCall((size_t) SystemCallNumber::WSBE, LINKCMD_RESUPPLY_DESKTOP, (size_t) args, (size_t) (antWhiteLine));
	memcpy(desktopBuffer + args[0], oldAntAreas[1], args[1] * 4);

	args[0] = (antMaxY - 1) * desktopWidth + antMinX;
	memcpy(oldAntAreas[2], desktopBuffer + args[0], args[1] * 4);
	SystemCall((size_t) SystemCallNumber::WSBE, LINKCMD_RESUPPLY_DESKTOP, (size_t) args, (size_t) (antWhiteLine));
	memcpy(desktopBuffer + args[0], oldAntAreas[2], args[1] * 4);

	args[0] += desktopWidth;
	memcpy(oldAntAreas[3], desktopBuffer + args[0], args[1] * 4);
	SystemCall((size_t) SystemCallNumber::WSBE, LINKCMD_RESUPPLY_DESKTOP, (size_t) args, (size_t) (antBlackLine));
	memcpy(desktopBuffer + args[0], oldAntAreas[3], args[1] * 4);

	for (int y = antMinY; y < antMaxY; ++y) {
		args[0] = y * desktopWidth + antMinX;
		args[1] = 2;
		args[2] = antMinX;
		args[3] = antMinX + 2;

		memcpy(moreOldAntStuff[0][y], desktopBuffer + args[0], 8);
		SystemCall((size_t) SystemCallNumber::WSBE, LINKCMD_RESUPPLY_DESKTOP, (size_t) args, (size_t) (y & 1 ? antBlackLine : antWhiteLine));
		memcpy(desktopBuffer + args[0], moreOldAntStuff[0][y], 8);

		args[0] = y * desktopWidth + antMaxX - 2;
		args[1] = 2;
		args[2] = antMaxX - 2;
		args[3] = antMaxX;
		memcpy(moreOldAntStuff[1][y], desktopBuffer + args[0], 8);
		SystemCall((size_t) SystemCallNumber::WSBE, LINKCMD_RESUPPLY_DESKTOP, (size_t) args, (size_t) (y & 1 ? antWhiteLine : antBlackLine));
		memcpy(desktopBuffer + args[0], moreOldAntStuff[1][y], 8);
	}


	prevAntScanlines[0] = antMinY;
	prevAntScanlines[1] = antMinY + 1;
	prevAntScanlines[2] = antMaxY;
	prevAntScanlines[3] = antMaxY - 1;

	prevAntMinX = antMinX;
	prevAntMaxX = antMaxX;
}

void delTree(char* basePath)
{
	char path[1000];
	memset(path, 0, 1000);
...
int main(int argc, char* argv[])
{
	...
	while (1) {
		...
		if (drawAnts) {
			...
			bool antXChanged = dummyWin.evnt.mouseX != antPrevX;
			bool antYChanged = dummyWin.evnt.mouseY != antPrevY;
			...
			if (antXChanged || antYChanged) {
				renderAnts(antXChanged, antYChanged, dummyWin.evnt.mouseX, dummyWin.evnt.mouseY, false);
		...
		if (dummyWin.evnt.type == EVENT_TYPE_KEYDOWN) {
			...
			if (dummyWin.evnt.key == (int) KeyboardSpecialKeys::Backspace || dummyWin.evnt.key == (int) KeyboardSpecialKeys::Delete) {
				for (int i = 0; i < MAX_DESKTOP_FILES; ++i) {
					if (!files[i].valid) continue;
					if (!files[i].selected) continue;

					char path[256];
					sprintf(path, "%s/%s", desktopBasePath, files[i].filepath);
