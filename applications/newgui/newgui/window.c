...
void Window_paint(Window* window, List* dirty_regions, uint8_t paint_children) {
    ...
    if (window->paint_function) {
        window->paint_function(window, window->paint_function_args);
    }
...
void Window_paint_handler(Window* window, void* v) {
    ...
    uint16_t mouse_y, uint8_t mouse_buttons)
{
    ...
    for (i = window->children->count - 1; i >= 0; i--) {
        ...
        if (!(mouse_x >= child->x && mouse_x < (child->x + child->width) &&
            ...
            //TODO: only do this when the mouse actually leaves

            if (child->mouseleave_function) {
                child->mouseleave_function(child, child->paint_function_args);
            }
    ...
    if (window->mouseup_function && !mouse_buttons/* && window->last_button_state*/) {
        window->mouseup_function(window, child->paint_function_args, mouse_x, mouse_y);
    }
    if (window->mousedown_function && mouse_buttons) {
        window->mousedown_function(window, child->paint_function_args, mouse_x, mouse_y);
    }
    if (window->mousemove_function) {
        window->mousemove_function(window, child->paint_function_args, mouse_x, mouse_y);
    }
