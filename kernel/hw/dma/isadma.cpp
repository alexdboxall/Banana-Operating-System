...
void DMAChannel::setLength(uint32_t len)
{
	...
	if (len > 0x10000) {
		panic("DMA TRANSFER LENGTH TOO LONG");
	}
}
...
void DMAChannel::start()
{
	...
	kprintf("DMA len 0x%X, addr 0x%X. addrreg 0x%X, countreg 0x%X, pagereg 0x%X\n", count, addr, addrReg, countReg, pageReg);
...
DMA::DMA(): Device("ISA DMA Controller")
{
	//8 bit ports
...
DMAChannel* DMA::lockChannel(int num)
{
	//try until you get it
	while (1) {
		kprintf("Trying to get channel 0x%X\n", num);
