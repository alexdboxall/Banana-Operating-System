...
void DMAChannel::setLength(uint32_t len)
{
	...
	if (len > 0x10000) {
		KePanic("DMA RANSFER LENGTH TOO LONG");
	}
}
...
void DMA8237::initialise()
{
	// regular registers
	hw->registerPortRange(0x00, 16, 1);
	hw->registerPortRange(0xC0, 32, 2);

	// 8 bit page registers
	hw->registerPortRange(0x81, 3, 1);
	hw->registerPortRange(0x87, 1, 1);

	// 16 bit page registers
	hw->registerPortRange(0x89, 3, 1);
	hw->registerPortRange(0x8F, 1, 1);
...
void DMA8237::deinitialise()
{
	
}

void DMA8237::setPowerState(DevicePowerState state)
{

}

const char* DMA8237::getHumanReadableName()
{
	return "Direct Memory Access Controller";
}


int DMA8237::getAvailable()
{
...
DMAChannel* DMA8237::tryLockChannelWithBase(int base)
{
...
DMAChannel* DMA8237::lockChannelWithBase(int base)
{
...
DMAChannel* DMA8237::lockChannel8()
{
	//8 bit uses channels 0-3, so use a base of 0
	return lockChannelWithBase(0);
}

DMAChannel* DMA8237::lockChannel16()
{
	//16 bit uses channels 4-7, so use a base of 4
	return lockChannelWithBase(4);
}

DMAChannel* DMA8237::lockChannel(int num)
{
	//try until you get it
	while (1) {
...
void DMA8237::unlockChannel(DMAChannel* channel)
{
	//check for null channel
	if (channel == nullptr) {
		KePanic("DMA UNLOCK BAD CHANNEL");
	}
	...
	if (!(available & (1 << channel->num))) {
		...
		KePanic("DMA CORRUPT BOOKKEEPING");
	}
