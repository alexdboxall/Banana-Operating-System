...
qemu-system-i386 -soundhw pcspk,ac97 -vga std -cpu max -serial file:log3.txt -m 18 -cdrom D:/Users/Alex/Desktop/banana-os/Installer/BANANA.ISO -fda mikeos.flp -rtc base=utc -d guest_errors,cpu_reset -monitor stdio  -drive file="banana-os/newimage.img",id=abcdefg,if=none -device ich9-ahci,id=ahci -device ide-drive,drive=abcdefg,bus=ahci.0  
pause
