...
qemu-system-i386 -soundhw pcspk -vga std -cpu max -serial file:log3.txt -m 8 -rtc base=localtime -d guest_errors,cpu_reset -monitor stdio -drive file=newimage.img,id=abcdefg,if=none -device ich9-ahci,id=ahci -device ide-hd,drive=abcdefg,bus=ahci.0 
pause
